...
fs::path CreatePackage(const fs::path& root, const std::string& component,
                       const std::string& version,
                       const std::vector<PackageFileSpec>& files,
  ...
  const auto manifest = manifest_override == nullptr
                            ? BuildManifest(component, version, files,
                                            package_dir, min_installer_version)
                            : *manifest_override;
...
void InstallInstallerRelease(const fs::path& root,
                             const std::string& version = "0.1.0",
                             const std::string& launcher_tag = "launcher") {
  const auto package_dir = CreateInstallerPackage(
      root / ("installer-" + version), version, launcher_tag);
  const auto result = zfleet::installer::ApplyPackage(root, package_dir);
  if (!result.ok) {
    FAIL(result.message);
  }
}
...
TEST_CASE(
    "apply accepts a .zip archive and preserves directory install checks") {
  const zfleet::test::ScopedTestDir test_root("installer");
  InstallInstallerRelease(test_root);
...
TEST_CASE(
    "apply fails on payload integrity mismatch without writing "
    "active-version") {
  const zfleet::test::ScopedTestDir test_root("installer");
  InstallInstallerRelease(test_root);
  ...
  SECTION("sha mismatch") {
    ...
    REQUIRE_FALSE(fs::exists(test_root / "agent" / "var" / "active-version"));
  }

  SECTION("size mismatch") {
    const auto package_dir = CreatePackage(
        test_root / "size-case", "agent", "0.1.0",
        {PackageFileSpec{.source_relative_path = "bin/zfleet_agent",
                         .target = "bin/zfleet_agent",
                         .content = "agent-binary",
                         .executable = true}});
...
TEST_CASE(
    "apply rejects component packages when active installer is missing or too "
    "old") {
  const zfleet::test::ScopedTestDir test_root("installer");
  ...
  SECTION("missing installer") {
    const auto result =
        zfleet::installer::ApplyPackage(test_root, agent_package);
    REQUIRE_FALSE(result.ok);
  ...
  SECTION("installer below min_installer_version") {
    ...
    const auto package_dir = CreatePackage(
        test_root / "agent-pkg-min", "agent", "0.2.0",
        {PackageFileSpec{.source_relative_path = "bin/zfleet_agent",
                         .target = "bin/zfleet_agent",
                         .content = "agent-binary",
                         .executable = true}},
...
TEST_CASE(
    "new version apply switches active-version and preserves old release") {
  const zfleet::test::ScopedTestDir test_root("installer");
  InstallInstallerRelease(test_root);
...
TEST_CASE(
    "apply from corrupt active to new version does not record corrupt "
    "previous") {
  const zfleet::test::ScopedTestDir test_root("installer");
  InstallInstallerRelease(test_root);
...
TEST_CASE("rollback failures keep active-version unchanged") {
  ...
  SECTION("invalid previous-version") {
    const auto package_v1 = CreatePackage(
        test_root / "v1", "agent", "0.1.0",
        {PackageFileSpec{.source_relative_path = "bin/zfleet_agent",
                         .target = "bin/zfleet_agent",
                         .content = "agent-binary-v1",
                         .executable = true}});
    ...
    REQUIRE_FALSE(rollback.ok);
    REQUIRE(ReadTextFile(ActiveVersionPath(test_root, "agent")) == "0.2.0\n");
    REQUIRE(ReadTextFile(PreviousVersionPath(test_root, "agent")) ==
            "../bad\n");
  }

  SECTION("previous-version matches active-version") {
    const auto package_dir = CreatePackage(
        test_root, "agent", "0.2.0",
        {PackageFileSpec{.source_relative_path = "bin/zfleet_agent",
                         .target = "bin/zfleet_agent",
                         .content = "agent-binary-v2",
                         .executable = true}});
