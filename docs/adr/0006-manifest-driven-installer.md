...
- [ADR 0015：launcher stub 归属 installer 的安装模型补充](0015-installer-owned-launcher-stub.md)
...
3. **版本目录 + active-version + launcher stub**  
   ...
   - launcher stub 的交付归属、版本语义与 `apply` 刷新责任由 [ADR 0015](0015-installer-owned-launcher-stub.md) 补充定义；
...
4. **发布包**  
  ...
  - manifest 必须声明目标 `platform` 与 `arch`，并可声明最低 `min_installer_version` 要求；installer 在安装 agent/server package 时必须拒绝不满足最低版本要求的 active installer；
...
5. **脚本职责**  
  `scripts/make-package.sh` / `install-local.sh` 仅负责编排构建、准备 payload、调用 C++ packager / installer；manifest 生成、摘要计算、归档读写、launcher stub 部署与安装/更新核心逻辑均由 C++ 程序承担。当前脚本只收集组件主程序二进制，动态库或其他依赖自动收集作为后续能力。
   launcher stub 在正式安装链路中的交付与刷新不再以本地脚本为契约，而由 [ADR 0015](0015-installer-owned-launcher-stub.md) 规定为 installer 语义的一部分。
