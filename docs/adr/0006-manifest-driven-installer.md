...
2. **组件按子目录隔离**  
   多组件可共用同一 `<root>/zfleet/`，但每个组件拥有独立子目录：

   - `<root>/zfleet/installer/`
   - `<root>/zfleet/agent/`
   - `<root>/zfleet/server/`
...
4. **发布包**  
   - agent/server 全量包只更新对应组件目录，可声明最低 `installer_version` 要求；  
...
5. **脚本职责**  
   `scripts/package.sh` / `deploy-local.sh` 仅负责构建、打 manifest、收集依赖与调用 installer，不实现安装/更新核心逻辑。

6. **可执行文件命名**  
   保持当前下划线形式：`zfleet_agent`、`zfleet_server`、`zfleet_installer`。

7. **manifest 最低安全契约**  
   manifest 至少声明组件、版本、目标相对路径、文件大小、SHA-256 摘要与可执行权限。installer 必须拒绝绝对路径、`..` 路径穿越、跨组件写入和未通过摘要校验的文件。签名与回滚保护在 v0.3 设计中补齐，但 manifest 结构需预留签名字段。

8. **事务与回滚语义**  
   `apply` 先写入 staging 目录并完成摘要校验，再原子切换为 `releases/<version>/`，最后通过临时文件 + rename 更新 `var/active-version`。上一版本至少保留到下一次成功启动验证之后；`rollback` 只切换到已存在且校验通过的相邻版本。
...
- 正向：安装、升级、回滚、本地调试与 CI 共用一套可测试逻辑；组件状态互不覆盖；跨平台行为一致；为 v0.3 签名与 Agent 子进程升级预留扩展点。  
- 负向：需实现 stub、installer 与打包管线；首次安装与 installer 更新依赖 installer-only 小包。  
- 后续工作：由 `docs/operations.md` 的部署、发布、升级、回滚章节承接，并按 P1–P4 分阶段实现；Agent 自更新与 Server 包下载接口在业务里程碑中实现。ADR 作为决策源，操作步骤和阶段实现计划由 `docs/operations.md` 承接；若后续部署文档明显膨胀或读者群分离，再拆出独立 deployment 文档。
