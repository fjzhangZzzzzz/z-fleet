...
| 二进制协议 / RPC | `protobuf`, `grpc` | v0.2+ | 按 ADR 0007 作为 Agent/Server 长期主控制通道落地 |
...
v0.1 使用 HTTP + JSON REST 验证最小闭环，Agent 主动出站连接 Server。长期主控制通道采用 [ADR 0007：Agent 控制通道采用 gRPC over HTTP/2](adr/0007-agent-control-channel-grpc-http2.md)，REST polling 仅作为过渡、诊断或兼容接口。
...
新增控制面能力应优先设计为 gRPC service/message，不再扩展 REST polling 或 WebSocket 作为长期主模型。
