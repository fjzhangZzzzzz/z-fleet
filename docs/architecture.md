...
| HTTP | `boost-beast` | Agent, Server | v0.1 REST 过渡接口；长期主控制通道见 ADR 0007 |
...
| HTTP/2 控制通道 | `nghttp2`, `protobuf-lite` | v0.2+ | 按 ADR 0007 作为 Agent/Server 长期主控制通道落地 |
...
v0.1 使用 HTTP + JSON REST 验证最小闭环，Agent 主动出站连接 Server。长期主控制通道采用 [ADR 0007：Agent 控制通道采用 HTTP/2 长连接与 protobuf-lite](adr/0007-agent-control-channel-grpc-http2.md)，REST polling 仅作为过渡、诊断或兼容接口。
...
当前任务下发过渡接口可使用 Agent 轮询，避免 Server 需要主动连回客户端：
...
新增控制面能力应优先设计为 HTTP/2 control message，不再扩展 REST polling、WebSocket 或 gRPC 作为长期主模型。
