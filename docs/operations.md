...
源码构建与终端首次安装的跨平台脚本边界评估见 [ADR 0012：跨平台构建、打包与安装脚本边界](adr/0012-cross-platform-script-boundary.md)。当前构建、测试和打包入口统一收敛到 Bash 脚本；Windows 本地通过 Git Bash 执行同一套入口。

组件级版本、tag 命名和 release note 生成策略以 [ADR 0014：组件级版本管理与标签驱动发布](adr/0014-component-versioning-and-tag-driven-release.md) 为决策源。

## 构建与测试

项目使用 CMake + vcpkg manifest mode。日常和 CI 应优先使用仓库脚本，避免本地命令与 CI 漂移。

Linux 下优先使用容器化构建入口 `scripts/builder.sh`，这样可以直接对齐 CI 的 Rocky Linux 构建环境，得到更一致的编译和测试结果。仅在需要调试宿主机环境、或容器化入口不可用时，再退回到宿主机构建入口 `scripts/build.sh`。Windows 下通过 Git Bash 使用同一套 Bash 脚本入口。

```bash
./scripts/builder.sh x64-linux-debug --test
./scripts/test.sh
```

未指定 preset 时，本地 Linux 默认使用 `x64-linux-debug`，Windows Git Bash 默认使用 `x64-windows-debug`。也可以显式指定：

```bash
./scripts/builder.sh x64-linux-release
./scripts/build.sh x64-linux-release
./scripts/test.sh x64-linux-release
```
...
当前 CI 已收敛为仅验证 `x64-linux-release` 和 `x64-windows-release`；`debug` 预设仅用于本地开发、排障和临时复现问题。`arm64-linux-*` 预设保留给 arm64 Linux 原生环境，不作为 x64 Linux 宿主上的跨架构构建入口。
...
```bash
./scripts/check.sh binaries --preset x64-linux-release
./scripts/check.sh packages --platform linux --arch x86_64 --build-type release <package.zip>...
```

Linux release 场景下，建议按以下顺序执行：先用 `scripts/builder.sh x64-linux-release` 构建，再运行 `scripts/check.sh binaries`，最后运行 `scripts/check.sh packages` 校验包产物。

## vcpkg

项目使用 vcpkg manifest mode。`scripts/build.sh` 会先 bootstrap 仓库内固定版本的 vcpkg tool，再由 `cmake --preset` 在 configure 阶段自动安装 manifest 依赖。
...
- `build/vcpkg_installed/<triplet>`：项目依赖安装目录，本地保留但 CI 不缓存。
- `.cache/vcpkg/archives`：项目 binary cache，本地与 CI 优先复用该缓存。

`vcpkg-configuration.json` 使用 builtin registry baseline 固定依赖版本。`overlay-ports` 和 `overlay-triplets` 仅在项目确实需要自定义 port 或 triplet 时引入，当前不作为日常依赖解析路径。
...
手动运行 `cmake --preset` 前，只需要确保 vcpkg tool 已 bootstrap；toolchain、triplet、installed dir 和 binary cache 均由 `CMakePresets.json` 固定：
...
z-fleet 采用组件级版本管理，`agent`、`server`、`installer` 分别维护自己的语义化版本号。发布时以组件 tag 作为版本确认依据，tag 格式为：

- `agent/v<semver>`
- `server/v<semver>`
- `installer/v<semver>`

其中 `<semver>` 必须与对应组件源码中的版本文件一致。版本文件属于源码约定，不由 CI 在发布时自动回写。

release note 采用 tag 范围自动生成草稿的方式：CI 读取上一个同组件 tag 到当前 tag 之间的提交记录，生成初稿后再由维护者人工确认和补充。具体发布规范见 [ADR 0014](adr/0014-component-versioning-and-tag-driven-release.md)。
...
```bash
./build/x64-linux-debug/apps/server/zfleet_server \
...
```bash
./build/x64-linux-debug/apps/agent/zfleet_agent --data-dir /tmp/zfleet-agent
```
...
./scripts/make-package.sh server installer --preset x64-linux-release --out /tmp/zfleet-packages --force
```

需要对已准备好的 payload 目录打包时，可直接调用 packager：

```bash
./build/x64-linux-debug/apps/packager/zfleet_packager pack \
  --component agent \
  --version 0.1.0 \
  --platform linux \
  --arch x86_64 \
...
```bash
./build/x64-linux-debug/apps/installer/zfleet_installer apply \
  --root ~/zfleet \
