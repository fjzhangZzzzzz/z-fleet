...
Linux 下优先使用容器化构建入口 `scripts/builder.sh`，这样可以直接对齐 CI 的 Rocky Linux 构建环境，得到更一致的编译和测试结果。仅在需要调试宿主机环境、或容器化入口不可用时，再退回到宿主机构建入口 `scripts/build.sh`。

```bash
./scripts/builder.sh linux-debug --test
./scripts/test.sh
```
...
```bash
./scripts/builder.sh linux-release
